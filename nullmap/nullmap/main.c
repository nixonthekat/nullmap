...
int main(int argc, char* argv[]) {
    ...
    if (!IsUserAnAdmin()) {
        ConsoleWarning("Not running as Administrator");
        ConsoleInfo("Some features may be limited without admin privileges");
        ConsoleInfo("For full functionality, restart as Administrator");
    } else {
        ConsoleSuccess("Running with Administrator privileges");
    }
    ...
    ConsoleInfo("[SERVICE] Active WinRing0 service: %s", GetCurrentServiceName());
    ...
    ConsoleInfo("[SERVICE] Verified WinRing0 service: %s", GetCurrentServiceName());

    // Test WinRing0 capabilities
    ConsoleInfo("Testing WinRing0 capabilities...");
    if (!TestWinRing0Access()) {
        ConsoleWarning("WinRing0 capabilities test had some issues, but continuing...");
    }

    ConsoleSuccess("nullmap initialized successfully");
    ConsoleInfo("[SERVICE] Ready with WinRing0 service: %s", GetCurrentServiceName());
    
    // Start RustClient monitoring and keep program alive
    ConsoleInfo("Starting RustClient.exe monitoring mode...");
    ConsoleInfo("[SERVICE] Monitoring with WinRing0 service: %s", GetCurrentServiceName());
    if (!WaitForRustClientAndStayActive()) {
        ConsoleError("Monitoring mode failed");
        return 1;
    }

    // This should never be reached due to infinite monitoring loop
    return 0;
}
